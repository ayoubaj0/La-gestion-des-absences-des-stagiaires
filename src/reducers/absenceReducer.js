import { ADD_ABSENCE, DELETE_ABSENCE, UPDATE_ABSENCE, SET_ABSENCES, SET_ABSENCE } from '../actions/absenceActions';
...
const absenceReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case DELETE_ABSENCE:
      ...
      return {...state, absences:[...state.absences.filter((absence)=>absence.id!==action.payload)]}
    case UPDATE_ABSENCE:
      return state.map(absence =>
        absence.id === action.payload.id ? action.payload.updatedAbsence : absence
      );
    default:
      return state;
  }
